...
function setupDeflyWalletConnectModalCloseListener(onClose: VoidFunction) {
	const deflyWalletConnectModalWrapper = document.getElementById(DEFLY_WALLET_CONNECT_MODAL_ID);
...
function openDeflyWalletSignTxnModal() {
	...
	return signTxnModal
		? waitForElementCreatedAtShadowDOM(signTxnModal, 'defly-wallet-sign-txn-modal__body__content')
		: Promise.reject();
}
