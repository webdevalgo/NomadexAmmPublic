...
function saveWalletDetailsToStorage(accounts: string[], type?: 'defly-wallet') {
...
function getWalletDetailsFromStorage(): DeflyWalletDetails | null {
	const storedWalletDetails = getLocalStorage()?.getItem(DEFLY_WALLET_LOCAL_STORAGE_KEYS.WALLET);
...
function getWalletConnectObjectFromStorage() {
	const storedWalletConnectObject = getLocalStorage()?.getItem(DEFLY_WALLET_LOCAL_STORAGE_KEYS.WALLETCONNECT);
