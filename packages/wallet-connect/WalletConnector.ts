...
import { base64ToUint8Array, composeTransaction, formatJsonRpcRequest } from './util/transaction/transactionUtils';
...
class WalletConnector {
	...
	connect() {
		return new Promise<string[]>(async (resolve, reject) => {
			try {
				...
				const { bridgeURL, shouldUseSound } = await getDeflyConnectConfig();
	...
	private async signTransactionWithMobile(signTxnRequestParams: DeflyWalletTransaction[]) {
		const formattedSignTxnRequest = formatJsonRpcRequest('algo_signTxn', [signTxnRequestParams]);
	...
	async signTransaction(txGroups: SignerTransaction[][], signerAddress?: string): Promise<Uint8Array[]> {
		...
		const signTxnRequestParams = txGroups.flatMap((txGroup) =>
			txGroup.map<DeflyWalletTransaction>((txGroupDetail) => composeTransaction(txGroupDetail, signerAddress))
		);
